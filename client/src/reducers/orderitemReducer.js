...
export default function orderitemReducer(
  ...
  switch (type) {
    case RETRIEVED_ORDER_ITEMS:
      return { ...state, isFetched: true, ...payload };
    case UPDATED_ORDER_ITEM:
      return { ...state, isUpdated: true, ...payload };
    case DELETED_ORDER_ITEM:
      const newDeleteState = Object.assign({}, state);
      newDeleteState.items = newDeleteState.items.filter(
        item => item.orderitem_id !== payload
      );
