...
class OrderItemView extends React.Component {
  ...
  componentDidMount() {
    this.props.onRetrievOrderItems(this.props.match.params.id).catch(error => {
  ...
  updateQuantity(e, id) {
    ...
    this.props.orderitems.items.map((item, index) => {
      if (item.orderitem_id === id) {
        ...
        this.props
          .onUpdateOrderItem(this.props.orderitems.items[index])
          .catch(err => {
      ...
      <div>
        ...
        <div>
              ...
              <thead className="black white-text">
                <tr className="table_row">
                  <th className="table_cell" scope="col">
                    <label className="th-label">Order ID</label>{" "}
                  </th>
                  <th className="table_cell" scope="col">
                    <label className="th-label">Creation Date </label>{" "}
                  </th>
                  <th className="table_cell" scope="col">
                    <label className="th-label">Status </label>
                  </th>
                  <th className="table_cell" scope="col">
                    <label className="th-label">Total Amount </label>
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr className="table_row">
                  ...
                  <td className="table_cell">
                    {dateFormat(
                      this.props.orderitems.created_date,
                      "dddd, mmmm dS, yyyy"
                    )}
                  </td>
          ...
          <div className="orderdetail-wrapper">
            {this.props.orderitems.status === "open" && (
              <div className="orderdetail-header">
                <button className="add_cart_btn" onClick={this.handleOpenModal}>
                  <i className="fa fa-cart-plus" /> Add to Order
                </button>
              </div>
            )}
                  ...
                  <tr className="table_row" key={index}>
                    ...
                    <td className="table_cell">
                      <input
                        ...
                        name={"input" + orderitem.orderitem_id}
                        ...
                        max="100"
                        onChange={e =>
                          this.updateQuantity(e, orderitem.orderitem_id)
                        }
                      />
                    </td>
                    ...
                    <td className="table_cell">
                      <button
                        ...
                        onClick={e =>
                          this.deleteCartItem(e, orderitem.orderitem_id)
                        }
                      >
